...
export default function ManageTopCategories({ hitCategory }) {
    ...
    // 2. New function to handle the confirmation prompt
    const handleInitiateDelete = (category) => {
        setCategoryToDelete(category);
        setIsDeleteModalOpen(true);
    };

    // 3. The actual delete function, now triggered by the modal's confirm button
    const handleDelete = async () => {
        ...
        try {
            const response = await fetch(`/api/top-category?id=${categoryToDelete.id}&publicId=${categoryToDelete.publicId}`, {
                method: 'DELETE',
            });
            ...
            setDeletingId(null);
            setCategoryToDelete(null); // Clear the stored category
        }
    };
    ...
    return (
        <div className="max-w-6xl mx-auto p-6">
            ...
            {/* 5. Render the delete confirmation modal */}
            {isDeleteModalOpen && (
                <ConfirmationModal
